...
| a write that touches several things on one agent | [guide](../objects/dashboard/guide.md) | layering onto a profile the caller does not own **surfaces the error and does not roll back** (`agents/templates/TemplateApplier.java:91`). Report per part instead — undoing half of it would remove things that may have been there first. |
