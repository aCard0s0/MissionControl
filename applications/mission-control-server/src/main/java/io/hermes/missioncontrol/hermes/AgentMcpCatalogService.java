...
public class AgentMcpCatalogService {
  ...
  public AgentProfileDto sync(String hostId, String containerId, String profile, String serverAlias) {
    ...
    String hostUrl = hosts.urlOf(hostId);
    AgentProfileDto current = profiles.get(hostUrl, containerId, profile);
    McpServerDto existing = current.mcp().stream()
  ...
  public AgentProfileDto enrich(String hostId, AgentProfileDto profile) {
    ...
    for (McpServerDto server : profile.mcp()) {
  ...
  private AddMcpServerRequest materialize(
      String agentHostId,
      String containerId,
      io.hermes.missioncontrol.mcp.McpServerDto source,
      String alias,
      boolean enabled) {
    String url = null;
    String command = null;
  ...
  private static AgentProfileDto copyWithMcp(AgentProfileDto value, List<McpServerDto> mcp) {
