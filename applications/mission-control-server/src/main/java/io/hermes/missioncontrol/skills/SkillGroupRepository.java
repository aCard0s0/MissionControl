  ...
  public void insert(SkillGroup group) {
    jdbc.update(
        "INSERT INTO skill_groups (id, name, description, skill_ids, guide_id, created_at, "
            + "updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
        group.id(), group.name(), group.description(), IdList.write(objectMapper, group.skillIds()), group.guideId(),
  ...
  public int update(SkillGroup group) {
    return jdbc.update(
        "UPDATE skill_groups SET name = ?, description = ?, skill_ids = ?, guide_id = ?, "
            + "updated_at = ? WHERE id = ?",
        group.name(), group.description(), IdList.write(objectMapper, group.skillIds()), group.guideId(),
